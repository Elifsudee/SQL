...
select A.ders_id, A.ders_ismi, A.ders_saati from qa_dersler as A--qa dersler esas tablo yaptiks
left join developer_dersler as B                                --soldaki tablo esas tablo             
on A.ders_id=B.ders_id                                      --left join dersek esas tablo sola yazilir

union

select B.ders_id, B.ders_ismi, B.ders_saati from qa_dersler as A
right join developer_dersler as B--right join ile esas tablom sagimda
on A.ders_id=B.ders_id;           --sagdaki tablo esas tablo
                             --right join yaparsa esas tablo saga yazilir
...
select film_name, category, actor_name from filmler as A--esas tablo solumda filmler
left join aktorler as B
on A.film_id=B.film_id;

--not:neden sutunlara gecici isim belrtmedik ayni isim de sutun yok karismasi mumkun degil

--2.yol


select film_name, category, actor_name from aktorler as B--esas tablo sagimda filmler
right join filmler as A
on A.film_id=B.film_id;

-- SORU2: Tüm actor_name'leri ve film_name'lerini listeleyiniz.

select actor_name, film_name from aktorler as A--esas tablo solumda aktorler
